<?php if(!defined('BASEPATH'))exit('No Direct Script Access Allowed'); 
	class Master_Product_Controller extends CI_Controller{
		...
		function index(){
			...
			if(empty($cek)){
				...
				$data['phpgrid'] = $this->ci_phpgrid->master_product();
				...
				$this->load->view('master/master_product_view',$data);
		...
		function export_excel(){
			...
			$objPHPExcel->setActiveSheetIndex(0)
                    ...
                    //Hello merupakan isinya
                                        ...
                                        ->setCellValue('A4', 'ID_PRODUCT')
										->setCellValue('B4', 'NAMA_PRODUCT')
										->setCellValue('C4', 'ID_TIPE_PRODUCT')
										->setCellValue('D4', 'HET')
										->setCellValue('E4', 'VOLUME')
										->setCellValue('F4', 'SATUAN')
										->setCellValue('G4', 'STATUS')
										->setCellValue('H4', 'PRODUCT_KODE_PRINCIPLE')
										->setCellValue('I4', 'DESCRIPTION_PRINCIPLE');
                    //mengisikan value pada tiap-tiap cell, A1 itu alamat cellnya 
                    //Hello merupakan isinya
			
			$query1 = $this->db->query("select * from Ms_PRODUCT ORDER by ID_PRODUCT desc");
			$i = 5;

			foreach($query1->result() as $prd){
				$objPHPExcel->setActiveSheetIndex(0)
							->setCellValue('A'.$i, $prd->ID_PRODUCT)
							->setCellValue('B'.$i, $prd->NAMA_PRODUCT)
							->setCellValue('C'.$i, $prd->ID_TIPE_PRODUCT)
							->setCellValue('D'.$i, $prd->HET)
							->setCellValue('E'.$i, $prd->VOLUME)
							->setCellValue('F'.$i, $prd->SATUAN)
							->setCellValue('G'.$i, $prd->STATUS)
							->setCellValue('H'.$i, $prd->PRODUCT_KODE_PRINCIPLE)
							->setCellValue('I'.$i, $prd->DESCRIPTION_PRINCIPLE);
            ...
            header('Content-Disposition: attachment;filename="msprodct"'.date('YMdHis').'".xls"');
            //unduh file
